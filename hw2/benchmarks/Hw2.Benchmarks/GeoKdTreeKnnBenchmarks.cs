...
public class GeoKdTreeKnnBenchmarks
{
    private const int KdTreeOperationsPerInvoke = 128;
    private const int FullScanOperationsPerInvoke = 16;
    ...
    public double QueryKdTreeKnn()
    {
        var total = 0.0;
        for (var i = 0; i < KdTreeOperationsPerInvoke; i++)
        {
            var query = _queries[_cursor++ % _queries.Length];
            var result = _index.SearchKNearest(query.Latitude, query.Longitude, K);
            total += result[0].DistanceMeters;
        }
    ...
    public double QueryFullScanKnn()
    {
        var total = 0.0;
        for (var i = 0; i < FullScanOperationsPerInvoke; i++)
        {
            var query = _queries[_cursor++ % _queries.Length];
            total += FullScanKnn(query.Latitude, query.Longitude, K);
        }

        return total;
    }

    private double FullScanKnn(double lat, double lng, int k)
    {
        var nearest = new List<double>(_points.Length);
        for (var i = 0; i < _points.Length; i++)
        {
            var point = _points[i];
            nearest.Add(HaversineMeters(lat, lng, point.Latitude, point.Longitude));
        }
    ...
    private static IEnumerable<GeoPoint> BuildPoints(int count)
    {
        for (var i = 0; i < count; i++)
        {
            ...
            var lng = -179.0 + (358.0 * ((i * 23) % 8_192) / 8_191.0);
