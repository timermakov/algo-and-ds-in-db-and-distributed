...

{
    ...
    private long _naivePostingBytes;
...

    {
        ...
        _naivePostingBytes = ComputeNaivePostingBytes(_memory);
...

    {
...

        {
            ...
            AppendCompressionStat(Case, bytes, _naivePostingBytes);
    ...
    public long NaivePostingBytes() => _naivePostingBytes;



    private static long ComputeNaivePostingBytes(InMemoryPositionalIndex memory)

    {

        long total = 0;

        foreach (var term in memory.Terms)

        {

            var postings = memory.GetPostings(term);
    ...
    private static void AppendCompressionStat(BenchCorpusCase benchCase, long segmentBytes, long naivePostingBytes)

    {
...

        {
            ...
            NaivePostingBytes = naivePostingBytes,

            CompressionRatio = naivePostingBytes > 0

                ? Math.Round(segmentBytes / (double)naivePostingBytes, 4)
...

    {
        ...
        public long NaivePostingBytes { get; set; }
