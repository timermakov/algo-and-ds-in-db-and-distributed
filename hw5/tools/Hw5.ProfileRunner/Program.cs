...
var mode = (GetArg(args, "--mode") ?? Environment.GetEnvironmentVariable("HW5_PROFILE_MODE") ?? "and").ToLowerInvariant();
...
var query = GetArg(args, "--query") ?? ModeQuery(mode);

var index = BuildIndex(docCount);
var (disk, segmentPath) = BuildDisk(index);
try
{
    ...
    {
        case "mmap":
            RunMmapLoop(disk, query, iterations);
            break;
        case "bm25":
            RunBm25Loop(index, query, iterations);
            break;
        default:
            RunAndLoop(index, query, iterations);
            break;
    }

    sw.Stop();
    Console.WriteLine($"Profile loop: mode={mode}, docs={docCount}, iterations={iterations}, elapsed={sw.ElapsedMilliseconds} ms");
...
static void RunAndLoop(InMemoryPositionalIndex index, string query, int iterations)
{
    var executor = new QueryExecutor();
    for (var i = 0; i < iterations; i++)
    {
        _ = executor.Execute(index, query).Matches.DocumentIds.Count;
    }
}

static void RunMmapLoop(DiskSegmentIndex disk, string query, int iterations)
{
    var executor = new QueryExecutor();
    for (var i = 0; i < iterations; i++)
    {
        _ = executor.Execute(disk, query).Matches.DocumentIds.Count;
    }
}

static void RunBm25Loop(InMemoryPositionalIndex index, string query, int iterations)
{
    var search = new SearchService();
    for (var i = 0; i < iterations; i++)
    {
        _ = search.Search(index, query, topK: 10, rankingMode: RankingMode.Bm25).Count;
    }
}

static string ModeQuery(string mode) => mode switch
{
    "bm25" => "alpha OR beta",
    "mmap" => "alpha AND beta",
    _ => "alpha AND beta",
...
static InMemoryPositionalIndex BuildIndex(int docCount)
{
...
static (DiskSegmentIndex Disk, string Path) BuildDisk(InMemoryPositionalIndex memory)
{
    var path = Path.Combine(Path.GetTempPath(), $"hw5-profile-{Guid.NewGuid():N}.seg");
